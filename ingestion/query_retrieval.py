...
class RetrievalQuery:
    ...
    def _get_metadata(self, doc_id):
        ...
        query = "SELECT source_path, page_number FROM mcq_metadata WHERE chunk_id = %s"
        cursor.execute(query, (doc_id,))
        result = cursor.fetchone()
        cursor.close()
        return result or {"source_path": "Unknown", "page_number": -1}
    ...
    def query(self, text, top_k=5):
        ...
        for doc, doc_id, score in zip(documents, ids, scores):
            ...
            structured_results.append({
                "text": doc,
                "document_id": doc_id,
                "document_name": metadata["source_path"],
...
def main():
    tester = RetrievalQuery(collection_name="159.341")
